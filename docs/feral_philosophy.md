...
> Why Feral is the way it is. Written for future contributors, so the
> answer to "should we remove X?" exists before the PR does. Not for
> end users — the user guide and marketing copy cover that.

---

## Positioning — what Feral is, in three generations

Feral is not "another agent". It is **the runtime that agents run
...
The runtime is foundation-model-agnostic: Feral Evolution Runtime +
...
- **The foundation model is a dependency, not a feature.** Feral
...
- **The Personal Agent is swappable.** Today's Personal Agent is
  ...
  calling Feral Evolution Runtime via API.

### What this is NOT

Feral is **not** AGI. Nothing in this architecture implies AGI, and
...
If asked "is Feral an AI agent?", the right answer is: "No — Feral is
the **runtime** that AI agents can run on, and that can make them
better over time in controlled ways."

If asked "is Feral an AGI project?", the right answer is: "No.
Feral's contribution is the bounded evolution mechanism, not any
...
- **ROS** in robotics. ROS is not a robot; ROS is the system robots
  run on. Feral Evolution Runtime is the system Personal Agents run
  on.
- **LLVM** in compilers. LLVM is not the final compiler; LLVM is the
  infrastructure compilers run on. Feral Evolution Runtime is the
...
what Feral is doing with their data.
...
is also how Feral can serve very different users from the same
foundation: their populations diverge over time.

## Why BRSI, not RSI?

Because unconstrained RSI is not safe, and Feral is positioned as
**research infrastructure**, not as a step toward AGI. The "B" is
the project's identity.

When a researcher reads "Feral is a recursive self-improvement
system", they think Yudkowsky, Bostrom, Anthropic's "When AI
Builds Itself". When they read "Feral is a **bounded** recursive
