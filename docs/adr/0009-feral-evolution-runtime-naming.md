# ADR-0009: Cinderpaw Evolution Runtime (FER) Naming
...
Cinderpaw is in a transitional moment. The existing docs use "Cinderpaw",
"Cinderpaw Agent", and "the agent" interchangeably. With BRSI in scope,
the project is no longer "another agent" — it is the **runtime that
agents run on**. Conflating the two causes:

1. **Wrong mental model.** A reviewer reads "Cinderpaw Agent evolves" and
   thinks "agent modifies its own weights". They miss that the
   runtime modifies the **policy on top of** the foundation model.
2. **Wrong category.** "Cinderpaw Agent" places Cinderpaw alongside Hermes
   Agent, OpenClaw, OpenHands, Cursor — AI agents with tools,
   memory, and planning. Cinderpaw's contribution is different: it is
   the **runtime** that those agents could run on, with bounded
   evolution built in.
3. **Wrong rebrand ceiling.** "Cinderpaw + Gemma" and "Cinderpaw + Qwen"
   are awkward if "Cinderpaw" is the agent. "Cinderpaw Evolution Runtime +
...
- **Cinderpaw Evolution Runtime (FER).** The runtime itself. What this
  doc and the codebase refer to as "Cinderpaw" when speaking
...
- **Personal Agent** (or "Personal AI"). The user-facing assistant
  ...
  the Cinderpaw sidecar agent loop; tomorrow it could be a different
  agent entirely (e.g., a third-party agent calling FER via API).

**Convention going forward:**

- "FER" or "Cinderpaw Evolution Runtime" → the runtime, the engine, the
...
- New docs and ADRs use "FER" / "Cinderpaw Evolution Runtime" for the
  system; "Personal Agent" for the user-facing AI.
- Existing docs (`rsi-evolution-spec.md`, the older
  `continual-personal-adaptation-plan.md`) still use "Cinderpaw Agent"
...
- Two names to maintain. Discipline required: docs that say "Cinderpaw
  Agent" when they mean the runtime are now imprecise.
- Users who already know Cinderpaw as "the agent" will need to
...
- `docs/cinderpaw_philosophy.md` (Positioning section — to be added)
