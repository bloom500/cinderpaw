...
`process.env.FERAL_*` at their call sites (see `CinderpawAgent/tests/config.test.ts`'s
...
| `FERAL_RATE_LIMIT_RPM` | int | `0` |  | Requests-per-minute cap applied to every inference endpoint, overriding the built-in published caps (NVIDIA NIM free tier = 40). 0 uses those defaults. Set this if you are on a paid tier with a different limit, or share one API key with something outside Cinderpaw. |
