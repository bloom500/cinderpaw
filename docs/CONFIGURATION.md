...
| `FERAL_FMS_ALLOW_CLOUD` | bool | `false` |  | Explicit opt-in for automatic FMS rebuilds and explicit FMS benchmarks to use cloud inference. Each path also requires its own positive USD cap. |
...
| `FERAL_FMS_BENCH_MAX_COST_USD` | string | `"0"` |  | Independent USD cap for an explicit FMS benchmark run. It never inherits the automatic rebuild budget; cloud still requires FERAL_FMS_ALLOW_CLOUD and exact pricing. |
