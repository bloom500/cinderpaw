...
- **`project_agent_cowork.md`** — Agent Cowork research baseline (Grok Bot /
  ...
  strictly reactive). S1→S4.6 shipped; `cowork_create_teammate` now creates
  teammates without script seeding. **Start from the current status at the
  top**; the old handoff's branch/uncommitted-file list is historical, not
  current working-tree state. **Read before any
