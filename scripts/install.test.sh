...
# A stranger sees our sentence, not "curl: (28) Failed to connect..." above it.
check "curl stays quiet on errors" "$(declare -f install_prebuilt cli_tag | grep -c -- "-fsSL")" "0"

# Until the first cinderpaw-agent-v* release exists, the one-liner must keep
# installing what it did before, not tell a stranger to check their internet.
no_release() (
  unset CINDERPAW_DOWNLOAD_BASE CINDERPAW_VERSION
  cli_tag() { return 1; }
  install_legacy() { echo legacy; }
  curl() { return "$REACHABLE"; }
  install_prebuilt 2>&1
)
check "no CLI release yet: old install" "$(REACHABLE=0 no_release)" "legacy"
check "offline: says offline"           "$(REACHABLE=1 no_release)" "$MSG_OFFLINE"
