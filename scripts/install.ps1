...
& {
  ...
  try {
    if ($env:CINDERPAW_DOWNLOAD_BASE) { $base = $env:CINDERPAW_DOWNLOAD_BASE }
      ...
      if (-not $tag) {
        # No cinderpaw-agent-v* release is published yet: install the desktop app,
        # as the README's Windows line did before this script. Stops applying on
        # its own the day the first CLI release exists.
        $a = (Invoke-RestMethod "https://api.github.com/repos/$repo/releases/latest").assets |
          Where-Object name -like '*x64-setup.exe' | Select-Object -First 1
        $setup = Join-Path $tmp $a.name
        Invoke-WebRequest -UseBasicParsing -Uri $a.browser_download_url -OutFile $setup
