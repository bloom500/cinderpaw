#!/usr/bin/env python3
"""Feral bundled LoRA trainer — implements the contract in docs/LORA_TRAINER.md.

Invocation (by Feral's CliTrainer):
    feral_lora_trainer finetune --base <gguf path|hf id> --data <pairs.jsonl> --out <dir> [--hyperparam v ...]
    feral_lora_trainer --version        (fast, no heavy imports)
    feral_lora_trainer self-test        (no-GPU logic check)
...
VERSION = "feral-lora-trainer 0.1.0"
...
def log(msg: str) -> None:
    print(f"[feral-lora-trainer] {msg}", flush=True)


def die(msg: str) -> "NoReturn":  # noqa: F821 - py3.10 compat, annotation only
    print(f"[feral-lora-trainer] FATAL: {msg}", file=sys.stderr, flush=True)
    sys.exit(1)
...
def load_pairs(data_path: str) -> list[dict]:
    """Read Feral's JSONL dataset: one {"prompt","response"} object per line."""
    pairs = []
    try:
...
def parse_hyperparams(extra: list[str]) -> dict:
    ...
    # crash a training cycle Feral queued. Warn and continue.
    if unknown:
...
def cmd_finetune(args: argparse.Namespace, extra: list[str]) -> None:
    ...
    # Contract metrics — captured by Feral for the human review card.
...
def main() -> None:
    ...
    if "--version" in argv:
        print(VERSION)
        return
    parser = argparse.ArgumentParser(prog="feral_lora_trainer")
