...
human one:
    ...
    python compare.py lifting-heavy-barbell   # reference | ours

Run them whenever an illustration is assigned to a state, and look before you
commit.

`compare.py` also shows the one thing we do NOT get for free. The pack draws the
character ON TOP of the object it is using -- two paws over the barbell, an arm
across the settings card -- so subtracting the character takes a bite out of the
prop with it. The barbell ships with two small gaps where the paws were. Filling
those holes was tried on 2026-09-10 and reverted: the same rule that closes a
gap in a bar also closes the deliberate gaps in dithered pixel art, and it
