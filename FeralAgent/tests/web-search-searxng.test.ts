...
describe("web_search / SearXNG", () => {
  ...
  test("with no backend configured it fails loudly instead of faking a search", async () => {
    const tool = createWebSearchTool({ searxngOrigin: null });
    const res = await tool.execute(
      { query: "current events" },
      // DDG instant-answer returns its usual nothing.
      ctxWith(async () => jsonRes({})),
    );

    expect(res.ok).toBe(false);
    expect(res.error).toBe("no_backend");
