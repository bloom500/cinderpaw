...
describe("loadUserConfig", () => {
  it("returns an empty default when no onboarding file exists", () => {
    ...
    expect(cfg.agentName).toBe("Feral");
  ...
  it("falls back to 'Feral' when agentName is missing or empty", () => {
    const dir = join(home, ".feral");
    mkdirSync(dir, { recursive: true });
    writeFileSync(
      join(dir, "onboarding.json"),
    ...
    expect(loadUserConfig(home).agentName).toBe("Feral");
  });
...
describe("buildUserPromptBlock", () => {
  ...
  it("uses the default name 'Feral' when the user did not pick one", () => {
    const cfg: UserConfig = {
      userName: "Darius",
      agentName: "Feral",
      hasOnboarded: true,
