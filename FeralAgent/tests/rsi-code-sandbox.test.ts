...
describe("integration — real git worktree lifecycle", () => {
  test("create → apply → measure → destroy against a throwaway repo", async () => {
    ...
    try {
      // Tiny repo shaped like the monorepo: FeralAgent/src/rsi/<file>.
      const run = (args: string[]) =>
        bunExec(["git", ...args], { cwd: repo, timeoutMs: 30_000 });
      await run(["init", "-q"]);
      await run(["config", "user.email", "t@t"]);
      await run(["config", "user.name", "t"]);
      mkdirSync(join(repo, "FeralAgent", "src", "rsi"), { recursive: true });
      writeFileSync(join(repo, "FeralAgent", "src", "rsi", "x.ts"), "export const x = 1;\n");
      await run(["add", "-A"]);
