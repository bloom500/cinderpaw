 ...
 *  `cubby` is the private local Feral instance, `paw` is the community support
...
function summarize(
  ...
  switch (type) {
    case "evolution.promoted":
      return `${where} promoted a candidate${fitness}.`;
    case "evolution.rejected":
      return `${where} evaluated a candidate${fitness} and kept the incumbent.`;
...
export function toPublicEvent(entry: unknown, publisher: Publisher): PublicEvent | null {
  ...
  const event: PublicEvent = {
    ...
    type,
    layer,
    summary: summarize(type, layer, metrics),
    candidateRef: typeof candidateId === "string" && candidateId ? publicRef(candidateId) : null,
