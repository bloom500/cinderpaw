/**
 * TUI rendering primitives — Feral headless slice 5.1.
 *
 ...
 *   text-muted     #8C7E6A
 *   brand          #C4843A   warm rust orange — the Feral accent
 *   error          #C0472A
 *   success        #6A9E5A
 *
...
/** Brand-orange coloured text. Used for the Feral wordmark + accents. */
...
// ---------------------------------------------------------------------------
// Prompt rendering — "You › " and "Feral › " lines
// ---------------------------------------------------------------------------
...
/**
 * Render the "Feral › " response marker. Used at the start of every
 * assistant message.
 */
export function renderFeralPrompt(useColor = shouldUseColor()): string {
  return `${primary("Feral ", useColor)}${muted("›", useColor)} `;
}
