...
import { tier0FloorBreach } from "./contract-leaves.ts";
import { defaultEnvelopesDir, updateEnvelope } from "./envelope-store.ts";
import type { EvalKind, EvalSpec } from "./eval-spec.ts";
...
export async function runModuleEval(
  ...
  const tier0Breach = tier0FloorBreach(candidateOutcomes);
