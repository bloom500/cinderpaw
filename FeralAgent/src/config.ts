...
export const CONFIG_SCHEMA: ConfigEntry[] = [
  ...
  { name: "FERAL_EXTERNAL_WRITE_BUDGET", type: "int", default: 50,
  ...
  { name: "FERAL_DRY_RUN", type: "bool", default: false,
  ...
  { name: "FERAL_WRITE_CONFIRM_HOSTS", type: "list", default: null,
  ...
  { name: "FERAL_TRUSTED_LOCAL_ORIGINS", type: "list", default: null,
    description: "Comma-separated exact origins (scheme+host+port) on loopback/private addresses that the SSRF guard may reach, for services the OPERATOR runs themselves. Exact-origin match only — trusting http://127.0.0.1:8080 does not trust any other local port — and the tool's own allowedDomains still applies. Extends the single FERAL_SEARXNG_URL exemption to any self-hosted backend.", security: true },
  ...
  { name: "FERAL_ATTACHMENT_MAX_CHARS", type: "int", default: 12_000,
  ...
  { name: "FERAL_FMS_ALLOW_CLOUD", type: "bool", default: false,
    description: "Explicit opt-in for automatic FMS rebuilds and explicit FMS benchmarks to use cloud inference. Each path also requires its own positive USD cap.", security: false },
  { name: "FERAL_FMS_AUTO_REBUILD_MAX_COST_USD", type: "string", default: "0",
    description: "Maximum cloud spend in USD for one automatic FMS tree rebuild. 0 keeps automatic rebuilds local-only; unknown cloud prices fail closed.", security: false },
  { name: "FERAL_FMS_BENCH_MAX_COST_USD", type: "string", default: "0",
    description: "Independent USD cap for an explicit FMS benchmark run. It never inherits the automatic rebuild budget; cloud still requires FERAL_FMS_ALLOW_CLOUD and exact pricing.", security: false },
  ...
  { name: "FERAL_CRON_JOB_TIMEOUT_MS", type: "int", default: 3_600_000,
