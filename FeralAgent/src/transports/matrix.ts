/**
 * Matrix — talk to Feral from any homeserver, including your own.
 *
...
export class MatrixConnector implements LiveConnector {
  ...
  async start(ctx: ConnectorContext): Promise<void> {
    ...
    if (!homeserver) {
      throw new Error(
        "matrix: no homeserver address — Feral cannot tell which server your account is on (expected MATRIX_HOMESERVER, e.g. https://matrix.org)",
      );
    }
