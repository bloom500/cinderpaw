...
export class TwitchConnector implements LiveConnector {
  ...
  async start(ctx: ConnectorContext): Promise<void> {
    ...
    // Feral already knows is asking them to do our bookkeeping.
    ...
    if (!this.#login) {
      throw new Error("twitch: the account name is missing — disconnect and pair again so Feral can read it back");
    }
