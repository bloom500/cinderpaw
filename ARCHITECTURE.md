...
each other directly. Full per-file layer map: `FeralAgent/src/rsi/README.md`.
...
| Faza 1 | sidecar/Rust bridge | (a) bridge client | (same) | `rsi/infra/bridge.ts`, `crates/feral-core/src/feral_agent.rs` |
...
2. **`FeralAgent/src/egress/inference-providers.ts`** — only if you
...
FeralAgent/src/egress/mcp-manager.ts         # connection owner (R5)
```
...
- New retriever (graph-of-leaves, hybrid) → add to
  ...
  (`FeralAgent/src/rsi/l1-config/genome.ts`); becomes a seam candidate.
...
the glossary in §5, and `FeralAgent/src/rsi/README.md`. Drift here is
