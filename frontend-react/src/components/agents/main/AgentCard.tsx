...
import { Trash2, AlertCircle, ChevronDown, Play, Square } from 'lucide-react';
...
function AgentRunPanel({ agent }: { agent: AgentConfig }) {
  ...
  const hasOutput = tokenText || items.length > 0 || runError;
    ...
    <div className="border-t border-border-subtle bg-bg-primary p-4 space-y-3">
      ...
      {hasOutput && (
        <div className="space-y-2 max-h-64 overflow-y-auto">
          ...
          {/* Top-level invoke error (e.g. no model loaded, agent not found) */}
          {runError && (
