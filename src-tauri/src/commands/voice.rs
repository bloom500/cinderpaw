...
#[tauri::command]
#[specta::specta]
// Merge note (17 Sep): `async` comes from the artifacts line (a keychain read
// belongs off the window's thread), `key_provider` from the voice line (the
// OpenRouter TTS id stores its key under "openrouter"). Both are needed.
