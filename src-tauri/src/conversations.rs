...
pub fn save_to_dir(dir: &Path, id: &str, title: &str, messages: &[PersistedMessage]) -> Result<()> {
    std::fs::create_dir_all(dir)?;

    let conv_path = dir.join(format!("{}.json", id));
    let created_at = if conv_path.exists() {
...
pub fn save(id: &str, title: &str, messages: &[PersistedMessage]) -> Result<()> {
    paths::ensure_dirs()?;
    save_to_dir(&paths::conversations_dir(), id, title, messages)
}
...
mod tests {
    ...
    fn save_creates_json_file_for_conversation() {
        let dir = tmp();
        save_to_dir(&dir, "c1", "Title", &msgs(3)).unwrap();
    ...
    fn two_conversations_persist_and_reload_independently() {
        ...
        save_to_dir(&dir, "conv-a", "Alpha", &msgs(3)).unwrap();
        save_to_dir(&dir, "conv-b", "Beta", &msgs(4)).unwrap();
    ...
    fn save_overwrites_messages_without_duplicating_index_entry() {
        ...
        save_to_dir(&dir, "c1", "Title", &msgs(1)).unwrap();
        save_to_dir(&dir, "c1", "Title", &msgs(3)).unwrap(); // update same conversation
    ...
    fn delete_removes_file_and_index_entry() {
        ...
        save_to_dir(&dir, "c1", "One", &msgs(2)).unwrap();
        save_to_dir(&dir, "c2", "Two", &msgs(2)).unwrap();
    ...
    fn full_roundtrip_simulating_app_restart() {
        ...
        save_to_dir(&dir, "session-1", "Hello world", &conv1_msgs).unwrap();
        save_to_dir(&dir, "session-2", "Tell me a joke", &conv2_msgs).unwrap();
