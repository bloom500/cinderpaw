// Draft workspace for the mascot redesign. Rendered via tools/render-mascot.ts.
export const FRAME_W = 16;
export const FRAME_H = 16;

export const PALETTE: Record<string, string | null> = {
  '.': null,
  k: '#26211f', // outline, pupils, mouth
  o: '#F57A1F', // main fur (brand orange)
  d: '#C2570F', // fur shade
  h: '#FFA14E', // fur highlight
  f: '#FFD9A8', // cream belly / inner ear / muzzle
...
// ---------------------------------------------------------------------------
// Base sprite: front-facing feral fox-monster. 16x16.
// Rows: 0-2 ears | 3-9 head (eyes 5-6, muzzle 7-9) | 10-13 body | 14-15 feet
// ---------------------------------------------------------------------------
const BASE: Frame = [
  '..o..........o..',
  '..oo........oo..',
...
/** Copy of `base` with `text` written at (row, col). */
function P(base: Frame, edits: Array<[row: number, col: number, text: string]>): Frame {
...
/** Deterministic dissolve: keep ~ratio of pixels (for spawn animation). */
function DIS(base: Frame, ratio: number): Frame {
  return base.map((row, r) =>
    row
      ...
      .map((ch, c) => {
        if (ch === '.') return ch;
        const h = (r * 31 + c * 17 + 7) % 100;
        return h < ratio * 100 ? ch : '.';
      })
      .join(''),
  );
}
...
type Edits = Array<[number, number, string]>;
...
const E_SLEEPY: Edits = [[5, 4, 'kk'], [5, 10, 'kk'], [6, 4, 'ww'], [6, 10, 'ww']];
...
const M_FLAT: Edits = [[8, 6, 'fkkf'], [9, 8, 'o']];
...
const I_EARTW = P(BASE, [[0, 2, '.'], [1, 1, 'o']]); // left ear flicks out
const I_YAWN = P(BASE, [...E_BLINK, ...M_OPEN]);

// ============================================================ TYPING (3) ===
// laptop: slate slab in front of body, rows 11-13
const LAPTOP: Edits = [
  [11, 3, 'eebbbbbbee'],
  [12, 2, 'eeeeeeeeeeee'],
  [13, 2, 'eeeeeeeeeeee'],
...
const D_CHECK_B = P(D_SMILE, [[2, 13, 'g'], [3, 14, 'g'], [1, 15, 'g'], [0, 15, '']]);
...
const RUN_2 = P(SH(RUN_1, 12, 14, 1), [
  [12, 0, '......oo..oo....'],
  [13, 0, '.....dd....dd...'],
  [14, 0, '................'],
]);
...
const W_BODY = P(SH(BASE, 0, 15, -1), [...E_HAPPY.map(([r, c, t]) => [r, c - 1, t] as [number, number, string]), ...M_SMILE.map(([r, c, t]) => [r, c - 1, t] as [number, number, string])]);
...
const SLP_B = P(SLP, [[12, 2, 'o'], [12, 13, 'o']]); // belly out
const Z1 = P(SLP, [[1, 13, 's']]);
const Z2 = P(SLP, [[1, 13, 's'], [0, 14, 'ss']]);
const Z3 = P(SLP, [[0, 14, 'ss'], [2, 12, 's']]);
const DRM_A = P(SLP, [[0, 12, 'ww'], [1, 11, 'wmmw'.replace('mm', 'mm')], [2, 12, 'ww']]);
const DRM_B = P(SLP, [[0, 12, 'ww'], [1, 11, 'wmmw'], [2, 12, 'ww'], [3, 10, 'w']]);

// ========================================================= SURPRISED (2) ===
const SURP = P(BASE, [...E_WIDE, [8, 6, 'fkkf'], [9, 7, 'kk']]);
const SURP_BANG_A = P(SURP, [[0, 7, 'y'], [1, 7, 'y'], [3, 7, '']]);
const SURP_BANG_B = P(SURP, [[0, 7, 'y'], [1, 7, 'y'], [2, 7, '.'], [0, 5, 'y'], [0, 9, 'y']]);

// =========================================================== CURIOUS (3) ===
const TILT = SH(BASE, 0, 9, 1);
const CUR_A = P(BASE, E_LOOK_L);
// peeking lean: tilted head + wide eyes
...
export const VARIANTS: Record<string, Frame[][]> = {
  idle: [
