...
func TestFetchConnectorCatalog_200(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Header version must match ConnectorCatalogVersionExpected (today: 6)
		// — the same constant the wizard bundle-side path asserts.
		w.Header().Set("X-Cinderpaw-Catalog-Version", "6")
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, payload)
	}))
	defer srv.Close()
...
func TestCatalogVersionExpectedPin(t *testing.T) {
	...
	if ConnectorCatalogVersionExpected != 6 {
		t.Fatalf("ConnectorCatalogVersionExpected = %d, want 6 (bump in lockstep with connectors::CONNECTORS_CATALOG_VERSION)", ConnectorCatalogVersionExpected)
	}
}
