...
//! `KeyLedger` — so a channel or provider Feral has never heard of shows up as a
...
use feral_core::migrate::{self, Found, KeyLedger, Plan};
...
pub fn run(
    ...
    if found.is_empty() {
        ...
        println!("Setting Feral up from scratch instead: feral setup");
        return 0;
    }
...
mod tests {
    use super::*;
    use feral_core::migrate::{Found, Plan, PlanItem, Source};
    use std::path::PathBuf;
    ...
    fn the_report_names_what_it_could_not_bring() {
        let plan = Plan {
            ...
            not_imported: vec!["channels.signal — no Feral connector for Signal yet".into()],
            conflicts: vec![],
