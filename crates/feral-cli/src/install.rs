...
//!
//! Feral arrives four different ways (npm, a from-source headless build, a
...
/// How Feral got onto this machine.
...
pub fn detect() -> Kind {
    ...
    // Sitting inside a checkout means this is `target/release/feral-cli`, i.e.
    // a build tree. Bail before anything below can offer to delete it.
    if let Some(tree) = exe.ancestors().find(|d| d.join("crates").join("feral-cli").is_dir()) {
...
pub fn update() -> i32 {
    ...
    match detect() {
        ...
        Kind::Dev { tree } => {
            ...
            eprintln!("         git -C {} pull && cargo build --release -p feral-cli", show(&tree));
            1
        }
...
pub fn uninstall(purge: bool, yes: bool) -> i32 {
    ...
    let kind = detect();
    let data = feral_core::paths::feral_dir();
    ...
    match &kind {
        ...
        Kind::MacApp => manual.push("rm -rf /Applications/Feral.app"),
        Kind::Dev { tree } => {
    ...
    if failed > 0 {
        if cfg!(windows) {
            eprintln!("\n{WARN}Windows cannot delete a running .exe — close Feral and remove the rest by hand.{RESET}");
        }
        return 1;
    }
    0
...
mod tests {
    ...
    fn classifies_the_packaged_installs() {
        let cases = [
            ("/home/d/.npm-global/lib/node_modules/feral-agent/vendor/feral-cli", Some(Kind::Npm)),
            (r"C:\Users\d\AppData\Roaming\npm\node_modules\feral-agent\vendor\feral-cli.exe", Some(Kind::Npm)),
            ("/Applications/Feral.app/Contents/MacOS/feral", Some(Kind::MacApp)),
            ("/usr/bin/feral", Some(Kind::SystemPackage)),
            // The two that must fall through to the filesystem probes.
            ("/home/d/.local/bin/feral", None),
            ("/home/d/src/feral/target/release/feral-cli", None),
        ];
    ...
    fn a_build_tree_is_never_an_install() {
        let dir = std::env::temp_dir().join("feral-uninstall-test/crates/feral-cli");
        std::fs::create_dir_all(&dir).unwrap();
        let tree = dir.parent().unwrap().parent().unwrap();
        let exe = tree.join("target/release/feral-cli");
        assert!(classify(&exe).is_none());
        assert!(exe.ancestors().any(|d| d.join("crates").join("feral-cli").is_dir()));
