...
//!   - Runtime boot forces `api_server_enabled` on for loopback consumers;
//!     the serialized settings default of `false` does not disable that boot path.
...
//!     the desktop token command, and external apps via `~/.cinderpaw/api-token`).
//!     Requests without the token cannot authenticate. Local processes able to
//!     read the token file can use the API; this is not same-user process isolation.
...
///      the canonical models directory. This rejects symlink escapes observed
///      during validation. The later path-based operation can still race with
///      filesystem changes; these checks do not eliminate TOCTOU races.
...
///
/// `api_key` is OPTIONAL. Omitted (or empty), the stored key for that
/// provider is used. That is what lets a caller list a configured
/// provider's models without re-prompting for a key the machine already
/// has — the TUI's model picker, the desktop settings pane and the
/// browser app all need the same thing, so the fallback lives here and
/// not in any one of them. A provider with no stored key gets the same
/// typed failure an empty pasted key would produce; we never probe with
/// an empty credential.
async fn providers_test(Json(req): Json<ProvidersTestReq>) -> Json<byok::TestProviderResponse> {
    let pasted = req.api_key.as_deref().map(str::trim).unwrap_or("");
    let key = if pasted.is_empty() {
        byok::byok_get(&req.provider_id).unwrap_or_default()
    } else {
        pasted.to_string()
    };
    if key.is_empty() {
        return Json(byok::TestProviderResponse {
            success: false,
            message: format!("No API key stored for {}.", req.provider_id),
            models: Vec::new(),
        });
    }
    Json(byok::test_provider(&req.provider_id, &key, req.base_url.as_deref()).await)
}

#[derive(serde::Deserialize)]
struct ProvidersTestReq {
    provider_id: String,
    /// Absent means "use the key already stored for this provider".
    #[serde(default)]
...
struct SaveByokReq {
    provider_id: String,
    enabled: bool,
    /// Absent means "keep the key already stored for this provider".
    #[serde(default)]
    api_key: Option<String>,
    base_url: Option<String>,
...
async fn runtime_byok_save(Json(req): Json<SaveByokReq>) -> Response {
    ...
    // `api_key` absent means "keep the key already stored". Changing only
    // the default model must not require the caller to hold the secret
    // again — the TUI's model picker, on a provider configured months ago,
    // has no key to re-send and must not wipe the one on disk.
    let api_key = match req.api_key.as_deref().map(str::trim) {
        Some(k) if !k.is_empty() => k.to_string(),
        _ => match byok::byok_get(&provider_id) {
            Some(k) => k,
            None => {
                return byok_save_error(
                    StatusCode::BAD_REQUEST,
                    "api_key_required",
                    "api_key is required — no key is stored for this provider yet",
                    None,
                )
            }
        },
