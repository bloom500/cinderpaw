...
/// actually been applied; `cinderpaw_agent::stdout_reader` looks up
...
/// long-lived handle next to `cinderpaw_agent_tx`.
...
impl RsiRequestRegistry {
    ...
    /// id). Used by `cinderpaw_agent::stdout_reader` to route engine
...
/// `pub` so future headless hosts built on `cinderpaw-core` can dispatch
...
/// **Visibility note (Faza 4.5 Slice 2 audit):** promoted from
/// `pub(crate)` (when this lived in `src-tauri/src/rsi/commands.rs`) to
/// `pub` so future headless hosts built on `cinderpaw-core` can commit
...
/// coming from the Cinderpaw Agent sidecar's stdout. Returns a JSON
