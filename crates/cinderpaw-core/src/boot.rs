//! Shared boot sequence — the runtime services every Cinderpaw host starts.
//!
//! Both the Tauri desktop entry point (`src-tauri/src/lib.rs`) and the
//! headless `cinderpaw-cli` gateway build the same runtime via this module.
...
//!     (via `cinderpaw_core::cinderpaw_agent::supervise`). Non-blocking: every
...
pub async fn start(
    ...
    // to check that no other Cinderpaw owns this brain. Passing it through means
    ...
    cinderpaw_agent::supervise(runtime, events, desktop_control, capabilities, admin, extra_bin_dirs);
}
...
/// Push the persisted settings to env vars the Cinderpaw Agent sidecar reads.
