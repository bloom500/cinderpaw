...
pub fn run(accept_risk: bool) -> i32 {
    ...
    if !acked && !accept_risk {
        ...
        println!("  {META}Cinderpaw is personal-by-default: it runs with your permissions, and a bad");
    ...
    for c in &candidates {
        ...
        if kind == "hardware_download" {
            // Cinderpaw-specific rung: local model a one-click download away.
        ...
        if kind == "existing_config" {
            ...
            println!(
                "  {WARN}Your already-configured model failed the test. Cinderpaw will not replace it automatically.{RESET}"
            );
            break;
        }
    }
    ...
    println!("  {META}stay cinderpaw. ↝{RESET}");
    0
}
