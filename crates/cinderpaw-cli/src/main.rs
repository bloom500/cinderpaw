...
/// Cinderpaw — your local AI runtime, headless. One brain, many faces: the same
...
enum Command {
    ...
    Stop,
    /// Update Cinderpaw to the latest release, then restart the gateway
    Update,
    ...
    /// Bring an existing OpenClaw or Hermes Agent setup into Cinderpaw
    Migrate {
...
fn main() {
    ...
    let code: i32 = match cli.command {
        ...
        // Both branch on how Cinderpaw was installed — npm, from source, or a
...
fn run_gateway() -> i32 {
    ...
    let code = rt.block_on(async {
        ...
        // another Cinderpaw host (desktop app or gateway) already owns this brain.
        ...
        let events: Arc<dyn cinderpaw_core::host::HostEvents> =
            Arc::new(cinderpaw_core::host::BroadcastEvents::new(runtime.events_tx.clone()));
...
async fn shutdown(runtime: &cinderpaw_core::runtime::RuntimeState) {
    *runtime.cinderpaw_agent_planned_exit.lock() =
        Some(cinderpaw_core::runtime::PlannedExit::shutdown());
    ...
    runtime.cinderpaw_agent_tx.lock().take();
    let child = runtime.cinderpaw_agent_process.lock().take();
