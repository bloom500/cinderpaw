...
//!     [`ProviderCatalogEntry`](feral_core::byok::ProviderCatalogEntry) or
//!     [`ConnectorCatalogEntry`](feral_core::connectors::ConnectorCatalogEntry)
...
//! `catalog_endpoints.rs` (which check `X-Feral-Catalog-Version` and
...
fn provider_catalog_matches_golden_snapshot() {
    ...
    assert_eq!(
        ...
        "Rust provider catalog drifted from canonical snapshot.\n\
         ...
         \n  UPDATE_CATALOG_GOLDEN=1 cargo test -p feral-core --test catalog_snapshots\n\
...
fn connector_catalog_required_fields_present() {
    for entry in connectors::connectors_catalog() {
        ...
        match entry.pairing_method {
            feral_core::connectors::PairingMethod::Qr => {
                assert!(
...
fn golden_files_in_sync_with_go_copy() {
    ...
    // Relative path from `crates/feral-core/tests/` up to the repo root
        ...
        let go_payload = std::fs::read_to_string(&go_path).unwrap_or_else(|e| {
            panic!(
                "missing Go-side golden copy at {go_path:?}. \
                 Run `cp crates/feral-core/tests/testdata/{name} tui/api/testdata/{name}` \
