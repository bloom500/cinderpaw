...
//! tests. Run via `cargo test -p feral-core --test catalogs`.
//!
...
//!   2. The `X-Feral-Catalog-Version` header (so a version drift shows
...
use feral_core::byok::{self, AuthStyle, ProviderCatalogEntry};
...
fn catalog_router() -> axum::Router {
    ...
    axum::Router::new()
        .route(
            ...
            get(feral_core::api::runtime_providers_catalog),
        )
        .route(
            "/runtime/connectors/catalog",
            get(feral_core::api::runtime_connectors_catalog),
        )
}
...
async fn providers_catalog_returns_200_with_version_header() {
    ...
    assert_eq!(
        ...
        "X-Feral-Catalog-Version must match byok::CATALOG_VERSION"
    );
}
...
async fn connectors_catalog_returns_200_with_version_header() {
    ...
    assert_eq!(
        ...
        "X-Feral-Catalog-Version must match connectors::CONNECTORS_CATALOG_VERSION"
    );
}
