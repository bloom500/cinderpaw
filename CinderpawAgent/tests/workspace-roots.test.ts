...
test("explicit CINDERPAW_WORKSPACE list is honored and scratch is always added", () => {
  ...
  const roots = loadWorkspaceRoots({ CINDERPAW_WORKSPACE: [a, b].join(delimiter) } as NodeJS.ProcessEnv);
  expect(roots).toContain(a);
  expect(roots).toContain(b);
  expect(roots).toContain(SCRATCH); // dedicated owned workspace always present
});

test("unset CINDERPAW_WORKSPACE defaults to cwd + home (broad by default)", () => {
...
test("ancestors of ~/.cinderpaw (home, drive root) are allowed as roots", () => {
  ...
  const roots = loadWorkspaceRoots({
    CINDERPAW_WORKSPACE: [homedir(), fsRoot].join(delimiter),
...
test("a root INSIDE ~/.cinderpaw is still dropped (brain exposure at registration)", () => {
  ...
  const roots = loadWorkspaceRoots({
    CINDERPAW_WORKSPACE: [rsiRepo, agentDir, realProject].join(delimiter),
  } as NodeJS.ProcessEnv);
...
test("a root inside EITHER profile dir is dropped, not just the current one", () => {
  ...
  for (const dir of dirs) {
    ...
    const roots = loadWorkspaceRoots({ CINDERPAW_WORKSPACE: inside } as NodeJS.ProcessEnv);
...
test("empty/whitespace segments never resolve to a root escape", () => {
  ...
  const roots = loadWorkspaceRoots({ CINDERPAW_WORKSPACE: ` ${blank} ` } as NodeJS.ProcessEnv);
