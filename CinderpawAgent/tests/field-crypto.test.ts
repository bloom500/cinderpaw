...
const prevKey = process.env.FERAL_DB_KEY;

afterEach(() => {
  if (prevKey === undefined) delete process.env.FERAL_DB_KEY;
  else process.env.FERAL_DB_KEY = prevKey;
});

describe("field-crypto (H-1)", () => {
  test("with a key: round-trips and the envelope hides the plaintext", () => {
    process.env.FERAL_DB_KEY = KEY_B64;
  ...
  test("without a key: stores plaintext (graceful no-op)", () => {
    delete process.env.FERAL_DB_KEY;
