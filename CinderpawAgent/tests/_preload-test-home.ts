 ...
 *
 * FERAL_HOME is the one variable every profile path derives from
...
// FERAL_HOME only, deliberately. `readEnv` resolves the modern CINDERPAW_*
// name FIRST and falls back to the legacy one, so setting CINDERPAW_HOME here
// would outrank every test that points FERAL_HOME at its own mkdtemp — and
...
if (!process.env.FERAL_HOME && !process.env.CINDERPAW_HOME) {
  process.env.FERAL_HOME = mkdtempSync(join(tmpdir(), "cinderpaw-test-home-"));
}
