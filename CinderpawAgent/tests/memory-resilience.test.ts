 ...
 * makes "Cinderpaw remembers me" something we can stand behind for every
 * shipping release.
 */
...
describe("memory/resilience: workspace switch", () => {
  ...
  test("workspaces use stable UUID identity; renaming does not change id", () => {
    ...
    const renamed = renameWorkspace(db.raw, ws.id, "Cinderpaw repo");
    expect(renamed?.id).toBe(ws.id);
    expect(renamed?.name).toBe("Cinderpaw repo");
    ...
    expect(list.find((w) => w.id === ws.id)?.name).toBe("Cinderpaw repo");
    db.close();
    teardown();
  });
