...
describe("toPublicEvent — mapping", () => {
  ...
  test("summary is generated, never copied from the row", () => {
    const event = toPublicEvent(journalRow(), "cubby")!;
    // The name is generated by public-event.ts from the candidate's hash, so
    // it belongs to the same class as the numbers and enum members here: not
    // text from the row.
    expect(event.summary).toBe(
      `Layer L1 evaluated ${event.candidateName} at fitness 0.500 and kept the incumbent.`,
    );
    expect(event.candidateName).toMatch(/^[A-Z][a-z]+$/);
  });

  test("the candidate's name is a function of its hash, not of its id", () => {
    // Why it matters: `candidateId` is attacker-influenceable in L3/L4, which
    // is why it is hashed before publishing. A name derived from the hash can
    // leak nothing the hash does not already — and a name that changed between
    // exports would rename the journal's own history every time it republished.
    const a = toPublicEvent(journalRow(), "cubby")!;
    const b = toPublicEvent(journalRow(), "cubby")!;
    expect(a.candidateName).toBe(b.candidateName);
    expect(candidateWord(a.candidateRef!)).toBe(a.candidateName);

    // Different candidates get different names (this pair does; the generator
    // can collide across a long journal, which is why candidateRef stays).
    const other = toPublicEvent(
      journalRow({ experimented: { candidateId: "some-other-candidate", layer: "L1" } }),
      "cubby",
    )!;
    expect(other.candidateName).not.toBe(a.candidateName);

    // Nothing of the raw id survives into what is published.
    expect(serialized(a)).not.toContain("seed-conservative");
  });
});
