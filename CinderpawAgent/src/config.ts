...
export const CONFIG_SCHEMA: ConfigEntry[] = [
  ...
  // ---- Connectors -------------------------------------------------------------------
  // (no FERAL_DISCORD_CLIENT_ID read in CinderpawAgent/src today — Rust/UI side.)
 ...
 *
 * Every `FERAL_*` variable is becoming `CINDERPAW_*`. Someone who put one in a
...
function readEnv(name: string): string | undefined {
  const modern = name.startsWith("FERAL_") ? `CINDERPAW_${name.slice(6)}` : name;
  const preferred = process.env[modern];
  if (preferred !== undefined) return preferred;
  const legacy = process.env[name];
  if (legacy !== undefined && modern !== name && !warnedLegacyNames.has(name)) {
    warnedLegacyNames.add(name);
    console.warn(
      `[cinderpaw] ${name} is the old name for ${modern} and still works, but it ` +
