...
export const CONFIG_SCHEMA: ConfigEntry[] = [
  ...
  { name: "CINDERPAW_ENABLE_DESKTOP_CONTROL", type: "bool", default: false,
    description: "Registers computer_use (OS accessibility-tree control). Off by default; set to \"true\" to enable.", security: true },
  { name: "CINDERPAW_DESKTOP_CONTROL_CONFIRM", type: "bool", default: true,
    description: "Per-action confirmation dialog for computer_use writes. On by default; set to \"false\" to disable (inverse-toggle var — see report for why this call site is not migrated to cfgBool).", security: true },
  ...
  { name: "CINDERPAW_DESKTOP_CONTROL_ALLOWED_APPS", type: "list", default: null,
    description: "Comma-separated allowlist of app names computer_use may target. Empty = fail closed. (Read by the Rust host, not CinderpawAgent/src.)", security: true },
  ...
  { name: "CINDERPAW_RSI_MAX_TOKENS", type: "int", default: null,
    description: "Token budget for one whole dream episode — every evaluation in it together, not one call. 2,000,000 when unset, and never above the governance policy's episode cap.", security: false },
  ...
  { name: "CINDERPAW_RSI_COOLDOWN_MS", type: "int", default: 600_000,
    description: "Minimum gap between the end of one dream episode and the start of the next automatic one, whatever the episode achieved. A dream you ask for skips it.", security: false },
  ...
  { name: "CINDERPAW_RSI_ERROR_THRESHOLD", type: "int", default: 3,
    description: "Errors within CINDERPAW_RSI_ERROR_WINDOW_MS that WAKE a dream episode (the error trigger). Clamped to at least 1.", security: false },
  ...
  { name: "CINDERPAW_RSI_STOP_ON_ACTIVITY", type: "bool", default: false,
    description: "Stop an automatic dream episode (idle, error or schedule trigger) once the user is active again; in-flight evaluations finish first. A dream you asked for keeps running.", security: false },
  ...
  { name: "CINDERPAW_INBOUND_HOST", type: "path", default: "127.0.0.1",
    description: "Address the webhook receiver binds to. Only open while a webhook connector (LINE) is enabled; serves only POST /connectors/<id>, never the gateway. 127.0.0.1 is reachable by a tunnel or reverse proxy on this machine; set 0.0.0.0 for a proxy on another box. TLS is the proxy's job.", security: true },
  { name: "CINDERPAW_INBOUND_PORT", type: "int", default: 18790,
    description: "Port for the webhook receiver (see CINDERPAW_INBOUND_HOST). Change it when another program has it; the connector says so on failure.", security: false },
